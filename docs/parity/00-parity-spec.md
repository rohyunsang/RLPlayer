...
| S39 | Copy the current subtitle line | P1 | triv | prop | M21 | `{"command":["get_property","sub-text"]}` → `await ctx.shell.copyText(...)`<br>variants `sub-text/ass`, `sub-text/ass-full`, `secondary-sub-text` | Returns an **empty string for image subtitles** — disable the action for those. **`ctx.shell.copyText` is ASYNC and it VERIFIES** — Electron 44's `clipboard.writeText` returns a Promise (this row read as synchronous until it was corrected) and it resolves even when the write does not land: measured on a box that denies clipboard access, `writeText` resolved and `readText()` came back empty while the app said "복사했습니다". `copyText` reads the clipboard back and throws; await it and show the error. Reflects the current subtitle "regardless of sub visibility", so it works with subs hidden. |
...
| C03 | Copy the current frame to the clipboard | P0 | triv | app | M22 | 1. `await ctx.mpv.command(['screenshot-to-file', tmpPng, 'subtitles'])`<br>2. `await ctx.shell.copyImagePng(bytes)` then `fs.rm(tmpPng)` | **CRITICAL — do not use `screenshot-raw`.** Verified twice (vo=null and vo=gpu-next): sending it over the JSON pipe makes **mpv exit with code 1**, killing playback, because `MPV_FORMAT_BYTE_ARRAY` has no JSON representation. It is client-API/Lua only. **CORRECTED — this row prescribed a method that does not exist.** It used to say `clipboard.writeImage(nativeImage.createFromPath(tmpPng))` and that `clipboard.writeImage(nativeImage)` is "simpler than the current Blob + ClipboardItem path". Verified against the RUNNING Electron 44 rather than its typings — `Object.getOwnPropertyNames(Object.getPrototypeOf(clipboard))` is `clear, has, read, readText, write, writeText` plus `selection`, and `typeof clipboard.writeImage` is `undefined`, as is `readImage`. Following the old row literally is a runtime `writeImage is not a function` on the first Ctrl+C, past typecheck and past any test that mocks Electron. M22 hit it, worked around it with `clipboard.write([new ClipboardItem({'image/png': blob})])`, and reported it; that workaround is now `ctx.shell.copyImagePng()` (§3.3.9) so no other module has to rediscover it. **`clipboard.writeText` is also a Promise in Electron 44** — S39, L25, L26 and L34 all name it and all read as synchronous; `ctx.shell.copyText()` is async and must be awaited. |
...
| L25 | Copy media info to clipboard | P1 | triv | app | M29 | `await ctx.shell.copyText(renderInfoAsText(snapshot))`, grouped `Key: value` lines | Include `hwdec-current`, `current-vo`, `video-params/pixelformat` and `mpv-version` — those four are what make a rendering bug report actionable. **`ctx.shell.copyText` is ASYNC and it VERIFIES** — see C03; a resolved `clipboard.writeText` is not evidence that the text is on the clipboard. |
| L26 | File properties dialog (our own) | P1 | md | app | M29 | `fs.statSync` (`size`, `birthtimeMs`, `mtimeMs`) + the probe snapshot; render in a second BrowserWindow mirroring the settings-window pattern; buttons `ctx.shell.showItemInFolder` and `await ctx.shell.copyText` | Build our own rather than shelling to Windows — PotPlayer does the same, and it lets us show media info the shell has no idea about. **`ctx.shell.copyText` is ASYNC and it VERIFIES** — see C03. |
...
| L30 | Thumbnail / cover-art view mode for playlist rows | P2 | lg | ext | M28 | CSS-grid mode switch in the playlist panel + `ctx.commands.invoke('nav-thumbnails.getThumb', {path})`, which caches into `<dataDir>/cache/thumbs`. Audio covers come from L29; **video thumbnails need M27's engine with video enabled**, which is a heavier probe than L22. | **Owner corrected.** This row said M27 and M27's `features` array listed it, but the rows it edits -- `src/renderer/src/features/playlist/` and the queue panel -- are M28's `ownedFiles`, so building it as written meant M27 editing M28's directory: exactly the collision the partition exists to prevent, written into the spec. The VIEW MODE is M28's (it is a mode of M28's list); the THUMBNAIL is M27's and arrives through M27's `getThumb` mediator. **Overlap warning stands:** N36 needs the same second-mpv-with-video infrastructure, and it is now `ctx.engine` in core -- one shared spawner, not two. |
...
| L34 | Copy path(s) / Reveal in Explorer | P1 | triv | app | M28 | `ctx.shell.showItemInFolder(item.path)`; `await ctx.shell.copyText(paths.join('\r\n'))` — **CRLF**, which is what Windows apps expect from a multi-line paste | Copying the actual **file** (CF_HDROP, pasteable into Explorer) is **not possible** with Electron's clipboard API. Offer "Copy path" only and use drag-out (L35) for real file transfer. **`ctx.shell.copyText` is ASYNC and it VERIFIES** — see C03; the CRLF join must be awaited and its rejection shown. |
